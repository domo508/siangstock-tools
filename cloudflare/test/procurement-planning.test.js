...
describe("採購規劃前台與入口", () => {
  ...
  it("重疊採購檔只計一次，並保留同一檔內的合法重複列", () => {
    const row = { documentCode: "PR1", supplier: "力榮", sku: "A1", name: "測試品", status: "主管審核", orderedQuantity: 10, deliveredQuantity: 0, remainingQuantity: 10, quantity: 10, unitCost: 100, orderedAmount: 1000, amount: 1000, purchaseDate: "2026-09-01", expectedDeliveryDate: "2026-09-15", receiptDate: "", documentClosed: false, fullyReceived: false, isCustomOrder: false, automaticProcurementExcluded: false };
    const summary = core.summarizePurchaseReports([{ records: [row, { ...row }] }, { records: [{ ...row }] }], "2026-09");
    expect(summary.pendingQuantity).toBe(20);
    expect(summary.pendingAmount).toBe(2000);
    expect(summary.duplicateRows).toBe(1);
  });

  it("分供應商ERP結案保留各自單號並連動協作批次", () => {
    const migration = readFileSync("worker/migrations/0029_procurement_erp_supplier_documents.sql", "utf8");
    const procurementWorker = readFileSync("worker/src/procurement.ts", "utf8");
    const toolApp = readFileSync("../procurement-planning/app.js", "utf8");
    expect(migration).toContain("CREATE TABLE procurement_batch_erp_documents");
    expect(migration).toContain("ALTER TABLE procurement_collaboration_drafts ADD COLUMN batch_id");
