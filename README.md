...
- `/store-transfer/`：門市週補貨與調撥；沿用主採購工具的公司Google OAuth與固定商品主檔／整體行銷策略來源，庫存、銷售及期間調撥仍在瀏覽器本機運算。結果分為一般必要補貨、單人被套前2名花色建議備貨、活動／贈品、提袋耗材及獨立缺貨未配；門市確認量與總部核准量會即時換算調撥後可售至。S品總倉5件內先依14／42／84天周轉保留，提袋累積至少4個可信週次後才以100個整箱建議。活動贈品會解析滿額／滿件、累贈規則，以門市POS平均客單、實際達標率、活動剩餘日及近21天耗用交叉估算。最後依門市拆分ERP檔。
