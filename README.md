...
- `/`：公司工具首頁。
- `/inventory/`：歷史庫存資料清洗。
- `/cost-analysis/`：庫存成本分析；在瀏覽器內整合八類 Excel 來源，比對 A 庫存推算耗用、B 淨銷售／跨體系成本與 C 非銷售調整。
...
所有同事都會使用 Cloudflare D1 中的公司集中規則。`/inventory/` 與 `/cost-analysis/` 都會在「開啟頁面」及每次按「開始分析」前，透過同源的 `/api/rules` 再取最新版；若規則服務無法使用，會直接禁止分析，不會偷偷改用舊規則。

- `/inventory/`：一般工具頁，可查看及下載目前規則，不能修改。
- `/inventory/rules-admin/`：規則管理頁；以公司 Google 帳號登入，只允許指定管理者。
- 管理頁可新增、刪除、匯入及匯出 JSON；發布時有版本衝突保護。

來源 Excel 不會被改寫。歷史庫存清洗會把排除資料保留在「排除項目」，庫存成本分析則會在「04_來源檢查」列出各報表排除前後的列數、數量、來源金額與逐列排除原因，方便稽核。
...
`_headers` 提供安全標頭。`/inventory/` 與 `/cost-analysis/` 使用同源集中規則服務；Excel 處理仍完全在瀏覽器本機完成。
