...
- `/store-transfer/`：門市週補貨與調撥；沿用主採購工具的公司Google OAuth與固定商品主檔／整體行銷策略來源，庫存、銷售及期間調撥仍在瀏覽器本機運算。一般補貨與活動／贈品調撥分開預覽、門市確認及總部核准；活動贈品會解析滿額／滿件、累贈規則，以門市POS平均客單、實際達標率、活動剩餘日及近21天耗用交叉估算，規則不足時改列人工確認。最後依門市拆分ERP檔。
