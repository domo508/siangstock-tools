...
這個首頁已預留成可持續擴充的公司工具入口。下次可用「**新增公司工具**」作為開頭，但不建議只給一個工具名稱；至少一起說明用途、輸入、輸出、資料能否上傳，以及這次要先規劃、做測試版或正式發布。
...
若有欄位規則、範例檔或理想畫面，也可以一併提供；沒有也沒關係，系統會先盤點並提出安全方案。根目錄的 `AGENTS.md` 已記錄給後續 Codex／開發者看的固定架構、安全界線、測試與發布流程。
...
測試涵蓋嚴格 JSON schema、未設定 Access 時拒絕管理寫入、無上傳 API、規則 API `no-store`、API 失敗禁止使用舊規則，以及既有分類、白名單、四張工作表、三張合計列與「實際庫存 × 進貨價」公式。
