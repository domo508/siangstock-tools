...
- `/procurement-planning/`：庫存採購規劃正式流程；原始 Excel 留在瀏覽器，固定 Google 來源唯讀取得，先產生母批次，再依每家實際供應商拆成獨立審核單位（普優瑪另拆天絲／天絲棉、長絨棉、無尺寸），各自完成回匯二次覆核、付款月份、核准摘要、集中額度台帳與 ERP 採購檔。門市週調撥核准後仍未配足的需求會置頂回拋，可併入下一張採購或另開補採新單；採購報表另列需求來源、最早需要日、未到貨最早到貨日、能否及時抵扣、時間缺口與本次新增量。只有已建立ERP且較新完整未到貨清單已不再出現的覆蓋量才轉為已到貨，後續週調撥部分／完整補配會累計並結案，人工結案或取消必須留下原因。SA／OA／SB／OB開頭品號固定排除一般採購與寄庫；其中OA41361、OA42361、OA43361、OA44361以掛(S)組合品理由排除。核准者完成計算後只會將月份成本彙總與截止日寫入公司共用快照，不會上傳原始Excel、檔名或逐列資料。
